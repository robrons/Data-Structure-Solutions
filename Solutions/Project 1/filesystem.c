...
char locnm[21];
char * remain = remLocNm(path,locnm);
fsNode * temp = getChild(fn->firstchild,locnm);
printf("%s\n",temp->c);
if(*remain!='\0') {
if(strcmp(temp->f,"F")==0) {
return 1;
...
SafeToCreate(temp,remain);
}
}

void deletetree(fsNode *n){
   if(n==NULL) return;
   deletetree(n->firstchild);
   deletetree(n->nextsibling);
   free(n);
}
